 ...
 */
public abstract class IOProgress implements Runnable {
    ...
    public void run() {
        ...
        while (!complete && attempt++ < attempts) {
            try (IOProcess ioProcess = process()) {
                ioProcess.process();
            } catch (Exception e1) {
    ...
    protected abstract IOProcess process();
    ...
    public static abstract class Builder<T extends IOProgress> {
